...
class PotentiateTests(unittest.TestCase):
    ...
    def test_on_zero_number_and_power(self) -> None:
        ''' 0 ^ 0 == 1 '''
        self.assertEqual(potentiate(0, 0, 1), 1)

    def test_on_zero_in_one(self) -> None:
        ''' 0 ^ 1 == 0 '''
        self.assertEqual(potentiate(0, 1, 1), 0)

    def test_on_zero_in_two(self) -> None:
        ''' 0 ^ 2 == 0 '''
        self.assertEqual(potentiate(0, 2, 1), 0)

    def test_on_one_in_zero(self) -> None:
        ''' 1 ^ 0 == 1 '''
        self.assertEqual(potentiate(1, 0, 1), 1)

    def test_on_one_in_two(self) -> None:
        ''' 1 ^ 2 == 1 '''
        self.assertEqual(potentiate(1, 2, 1), 1)

    def test_on_two_in_two(self) -> None:
        ''' 2 ^ 2 == 4 '''
        self.assertEqual(potentiate(2, 2, 1), 4)

    def test_on_three_in_five(self) -> None:
        ''' 3 ^ 5 == 243 '''
        self.assertEqual(potentiate(3, 5, 1), 243)
    ...
    def test_on_large_random_input(self) -> None:
        ...
        for _ in range(subtest_number):
            ...
            with self.subTest():
                self.assertEqual(potentiate(number, exponent, 1), number ** exponent)
